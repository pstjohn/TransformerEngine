/*************************************************************************
 * Copyright (c) 2022-2026, NVIDIA CORPORATION & AFFILIATES. All rights reserved.
 *
 * See LICENSE for license information.
 ************************************************************************/

#include <string>

#include "../extensions.h"
#include "../stable_common.h"
#include "transformer_engine/transformer_engine.h"

// =============================================================================
// Category A recipe functions ported to stable ABI
// =============================================================================

namespace transformer_engine::pytorch::stable {

using Tensor = torch::stable::Tensor;

void compute_amax(Tensor tensor, Tensor amax) {
  auto input_tensor = torch::stable::contiguous(tensor);
  const auto te_input = makeTransformerEngineTensor(input_tensor);

  NVTE_CHECK(amax.scalar_type() == ScalarType::Float, "amax must be a float tensor");
  NVTE_CHECK(amax.numel() == 1, "amax must have exactly one element");
  auto* amax_ptr = reinterpret_cast<float*>(amax.data_ptr());
  transformer_engine::TensorWrapper fake_te_output(
      /*dptr=*/nullptr, te_input.shape(), transformer_engine::DType::kFloat32, amax_ptr);

  nvte_compute_amax(te_input.data(), fake_te_output.data(),
                    getCurrentCUDAStreamRaw(tensor.get_device_index()));
}

void fused_amax_and_scale_update_after_reduction(Tensor amax_reduction_buffer,
                                                 std::vector<Tensor> amax_histories,
                                                 std::vector<Tensor> scales,
                                                 std::string amax_compute_algo, int64_t fp8_dtype,
                                                 double margin) {
  size_t num_tensors = amax_histories.size();
  std::vector<NVTETensor> te_amax_histories;
  std::vector<NVTETensor> te_scales;
  te_amax_histories.reserve(num_tensors);
  te_scales.reserve(num_tensors);
  for (size_t i = 0; i < num_tensors; i++) {
    te_amax_histories.push_back(nvte_create_tensor(NVTE_DELAYED_TENSOR_SCALING));
    NVTETensor& amax_history = te_amax_histories.back();
    auto ah_sizes = amax_histories[i].sizes();
    std::vector<size_t> ah_shape_vec(ah_sizes.begin(), ah_sizes.end());
    NVTEShape amax_shape = nvte_make_shape(ah_shape_vec.data(), ah_shape_vec.size());
    NVTEBasicTensor amax_history_data = {
        amax_histories[i].data_ptr(), static_cast<NVTEDType>(transformer_engine::DType::kFloat32),
        amax_shape};
    nvte_set_tensor_param(&amax_history, kNVTERowwiseData, &amax_history_data);

    te_scales.push_back(nvte_create_tensor(NVTE_DELAYED_TENSOR_SCALING));
    NVTETensor& scale = te_scales.back();
    auto sc_sizes = scales[i].sizes();
    std::vector<size_t> sc_shape_vec(sc_sizes.begin(), sc_sizes.end());
    NVTEShape scale_shape = nvte_make_shape(sc_shape_vec.data(), sc_shape_vec.size());
    NVTEBasicTensor scale_data = {scales[i].data_ptr(),
                                  static_cast<NVTEDType>(transformer_engine::DType::kFloat32),
                                  scale_shape};
    nvte_set_tensor_param(&scale, kNVTERowwiseData, &scale_data);
  }
  nvte_delayed_scaling_recipe_amax_and_scale_update_after_reduction(
      makeTransformerEngineTensor(amax_reduction_buffer).data(), te_amax_histories, te_scales,
      amax_compute_algo.c_str(), static_cast<NVTEDType>(fp8_dtype), static_cast<float>(margin),
      getCurrentCUDAStreamRaw(amax_reduction_buffer.get_device_index()));
  for (auto& t : te_amax_histories) {
    nvte_destroy_tensor(t);
  }
  for (auto& t : te_scales) {
    nvte_destroy_tensor(t);
  }
}

}  // namespace transformer_engine::pytorch::stable

STABLE_TORCH_LIBRARY_IMPL(transformer_engine, CUDA, m) {
  m.impl("compute_amax", TORCH_BOX(&transformer_engine::pytorch::stable::compute_amax));
  m.impl(
      "fused_amax_and_scale_update_after_reduction",
      TORCH_BOX(&transformer_engine::pytorch::stable::fused_amax_and_scale_update_after_reduction));
}
