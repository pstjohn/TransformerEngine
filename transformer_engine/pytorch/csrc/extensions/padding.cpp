...
void fused_multi_row_padding(Tensor input, Tensor output, std::vector<int64_t> input_row_list,
  ...
  for (size_t tensor_id = 0; tensor_id < num_tensors; ++tensor_id) {
    ...
    const size_t input_dptr_offset =
        static_cast<size_t>(input_row_list[tensor_id]) * input.sizes()[1] * input.element_size();
    input_char_ptr += input_dptr_offset;
    d_input_ptr = reinterpret_cast<void*>(input_char_ptr);

    input_shape_list.push_back(
        {static_cast<size_t>(input_row_list[tensor_id]), static_cast<size_t>(input.sizes()[1])});
    ...
    const size_t output_dptr_offset = static_cast<size_t>(padded_input_row_list[tensor_id]) *
                                      output.sizes()[1] * output.element_size();
    ...
    output_shape_list.push_back({static_cast<size_t>(padded_input_row_list[tensor_id]),
                                 static_cast<size_t>(output.sizes()[1])});
  }
  ...
  auto make_tensor = [&tensor_wrappers](void* dptr, const std::vector<size_t>& shape,
    ...
    tensor_wrappers.emplace_back(transformer_engine::TensorWrapper(dptr, shape, dtype));
...
STABLE_TORCH_LIBRARY_IMPL(transformer_engine, CUDA, m) {
  m.impl("fused_multi_row_padding",
         TORCH_BOX(&transformer_engine::pytorch::stable::fused_multi_row_padding));
  m.impl("fused_multi_row_unpadding",
         TORCH_BOX(&transformer_engine::pytorch::stable::fused_multi_row_unpadding));
}
